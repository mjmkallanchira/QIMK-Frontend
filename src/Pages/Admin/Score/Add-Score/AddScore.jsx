  ...
  const getteamdata = async () => {
    try {
      ...
      if (response.ok) {
        ...
        settotalpoint(data.stagepoint + data.offstagepoint);
            ...
            return (
              <div className="col-12 my-5 col-lg-5 ">
                <div className="  text-decoration-none notes p-2   flex flex-col flex-nowrap content-center justify-around items-center w-full    bg-gray-800 border border-gray-800 shadow-lg  rounded-2xl">
                  <img src="/books.png" alt="" className="w-25 my-2" />
                  <div className="text-white text-2xl  font-bold text-center  mt-2 ">
                    {obj.name}
                  </div>
                  <div className="text-info text-lg mt-2 font-bold text-center   ">
                    Total Points :{" "}
                    <span className="text-lime-400">
                      {obj.stagepoint + obj.offstagepoint}
                    </span>
                  </div>
                  <hr className="text-white w-full" />
                  <div className="flex w-full justify-around align-middle content-center">
                    <div className="text-success  text:sm font-bold text-center   ">
                      Stage Points :{" "}
                      <span className="text-green-400">{obj.stagepoint}</span>
                    </div>
                    <div className="text-danger text-md font-bold text-center   ">
                      offstage Points :{" "}
                      <span className="text-red-400">{obj.offstagepoint}</span>
                    </div>
                  </div>

                  <Link
                    to={`/admin/add-score/change-score/${obj._id}/${obj.name}/${obj.stagepoint}/${obj.offstagepoint}/`}
